...
half4 DeferredReflection(UnityStandardData data, float3 worldPos)
{
...
half4 frag (unity_v2f_deferred i) : SV_Target
{
	...
	if (_MsaaFactor <= 1)
	{
		...
		return DeferredReflection(data, worldPos);
	}
	else
	{
		...
		for (a = 0; a < msaaCount; a++)
		{
			...
			col += DeferredReflection(data, worldPos);
		}
		return col / msaaCount;
	}
}
